...
import os, json, time, queue, signal, argparse
        ...
        while not stop:
            ...
            while True:
                ...
                if allowed_lateness == 0 and win < newest_win:
                    print(f"LATE-DROP [{fmt_hms(win)}–{fmt_hms(win+window_sec)}) user={key[0]} url={key[1]} bytes={rec['bytes']}")
                    continue
            ...
            for w in sorted(to_flush):
                for (user_id, url), total in sorted(windows[w].items()):
                    ...
                    print(f"{prefix} [{fmt_hms(w)}–{fmt_hms(w+window_sec)}) user={user_id} url={url} bytes={total}")
                printed.add(w)
                del windows[w]
...
if __name__ == "__main__":
    ...
    ap.add_argument("--allowed_lateness", type=int, default=0)
    ap.add_argument("--window", type=int, default=60)
    ap.add_argument("--max_runtime", type=int, default=0)
    args = ap.parse_args()
    run(allowed_lateness=args.allowed_lateness, window_sec=args.window, max_runtime=args.max_runtime)
'@ | Out-File -Encoding UTF8 .\assignment.py
